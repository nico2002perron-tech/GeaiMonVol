...
function gen(region, ans, gs, acc) {
    ...
    return {
        title: `7 jours magiques en ${region}`, subtitle: REGIONS.find(r => r.name === region)?.desc || "",
...
function scoreR(ans) {
    const tags = []; Object.values(ans).forEach(v => { if (Array.isArray(v)) tags.push(...v); else tags.push(v) });
...
export default function QuebecPlanner({ onClose }) {
    ...
    const doSwap = (di, sl, reason) => {
        const cur = itin.days[di][sl]; const c = reason === "trop_cher" ? .4 : reason === "calme" ? .7 : 1.1;
        setSwap(p => ({
            ...p, alts: [
    ...
    const confSwap = (di, sl, alt) => {
        setItin(p => {
            const u = JSON.parse(JSON.stringify(p)); u.days[di][sl] = alt; const d = u.days[di];
            ...
            u.total_cost_per_person = u.days.reduce((s, dd) => s + (dd.daily_cost_per_person || 0), 0); return u
        }); setSwap(null)
    };

    const exportJ = () => {
        const j = { region, answers: ans, itinerary: itin, feedback: fb, notes, groupSize: gs, exportDate: new Date().toISOString(), version: "GeaiMonVol-v4" };
...
.qS::-webkit-scrollbar{width:3px}.qS::-webkit-scrollbar-thumb{background:rgba(46,125,219,.1);border-radius:3px}`;
    ...
    const Meal = ({ data, label, icon, color, di, sl }) => {
    ...
    return (
        <div onClick={e => { if (e.target === e.currentTarget && onClose) onClose() }} style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,.45)", backdropFilter: "blur(14px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 12, fontFamily: "'Fredoka',sans-serif" }}>
            ...
            <div className="qS" ref={sr} style={{ width: "100%", maxWidth: step === "result" ? 780 : step === "ranking" ? 600 : step === "accommodation" ? 560 : 520, maxHeight: "94vh", overflow: "auto", background: "linear-gradient(175deg,#F8FAFF,#EDF2FB,#E4EAF6)", borderRadius: 28, border: "1px solid rgba(46,125,219,.08)", boxShadow: "0 32px 80px rgba(0,0,0,.2),inset 0 0 0 1px rgba(255,255,255,.5)", transition: "max-width .5s cubic-bezier(.25,.46,.45,.94)", position: "relative" }}>
                ...
                {onClose && <button onClick={onClose} style={{ position: "absolute", top: 14, right: 14, zIndex: 10, width: 32, height: 32, borderRadius: "50%", border: "none", background: "rgba(26,58,107,.06)", color: "rgba(26,58,107,.4)", fontSize: 15, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Fredoka',sans-serif" }}>✕</button>}
                ...
                {step === "accommodation" && <div style={{ padding: "26px 24px 32px", animation: "qF .4s ease" }}>
                    ...
                    {accList.length > 0 ? <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        ...
                        <p style={{ color: "#5A6B80", fontSize: 13 }}>Pas d'hébergements pré-sélectionnés.</p>
                        <button onClick={() => pickA({ name: "Hébergement à déterminer", type: "Flexible", cost: Math.round(100 * (budget === "econome" ? .6 : budget === "luxe" ? 1.5 : 1)) })} style={{ marginTop: 10, padding: "10px 20px", borderRadius: 100, border: "none", background: "linear-gradient(135deg,#2E7DDB,#1A3A6B)", color: "white", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "'Fredoka',sans-serif" }}>Continuer →</button></div>}
